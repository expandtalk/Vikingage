...
-- Kung -> dynasti ur historical_kings.dynasty_id. Bara kungar vars dynasti-nod finns.
...
WHERE k.dynasty_id IS NOT NULL
  AND EXISTS (SELECT 1 FROM entity_registry er WHERE er.id = k.dynasty_id AND er.entity_type = 'dynasty')
...
-- Kyrka -> socken (befintlig FK parish_id).
INSERT INTO relationship (subject_id, predicate, object_id, source_ref, confidence)
...
-- Kyrka -> härad (befintlig FK hundred_id).
INSERT INTO relationship (subject_id, predicate, object_id, source_ref, confidence)
SELECT c.id, 'part_of_hundred', c.hundred_id, 'ecclesiastical_sites.hundred_id', 'certain'
FROM ecclesiastical_sites c
WHERE c.hundred_id IS NOT NULL
  AND EXISTS (SELECT 1 FROM entity_registry er WHERE er.id = c.hundred_id AND er.entity_type = 'hundred')
ON CONFLICT DO NOTHING;
```
