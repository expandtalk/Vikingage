      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <Section id="vad" defaultOpen icon={<Tag className="h-5 w-5 text-gold" />}
          ...
          <div className="space-y-3 text-sm text-muted-foreground max-w-3xl">
            <p>
              {sv
                ? <>Grundordet är fornnordiska <em>tun</em> ’gärdesgård’ → ’det inhägnade området’ (samma ord som tyska <em>Zaun</em> och engelska <em>-ton</em>/<em>town</em>). <strong className="text-foreground">Tuna</strong> är en särskild neutral pluralform som i Sverige fått en specialfunktion som namn på <strong className="text-foreground">centralorter</strong>. Över hälften av alla -tuna-namn är (eller har varit) osammansatt <em>Tuna</em>. <Status kind="belagt" sv={sv} /></>
                : <>The base word is Old Norse <em>tun</em> ’fence’ → ’the enclosed ground’ (the same word as German <em>Zaun</em> and English <em>-ton</em>/<em>town</em>). <strong className="text-foreground">Tuna</strong> is a distinct neuter plural that in Sweden took on a special function as a name for <strong className="text-foreground">central places</strong>. Over half of all -tuna names are (or were) the simplex <em>Tuna</em>. <Status kind="belagt" sv={sv} /></>}
            </p>
            <p>
              {sv
