...
const KalmarMap: React.FC<{ places: PlaceName[]; harbor: Harbor | null; coins: Coin[] }> = ({ places, harbor, coins }) => {
  ...
  useEffect(() => {
    ...
    places.filter((p) => p.lat != null && p.lng != null).forEach((p) => {
      ...
      const color = husaby ? '#f59e0b' : '#a78bfa';
      pts.push([p.lat!, p.lng!]);
      L.circleMarker([p.lat!, p.lng!], { radius: husaby ? 9 : 6, color, weight: 2, fillColor: color, fillOpacity: husaby ? 0.35 : 0.6 })
        .bindTooltip(p.name, { permanent: husaby, direction: 'top', offset: [0, -8], className: 'ang-clabel' })
        .bindPopup(`<b>${p.name}</b> <span style="font-size:10px;color:#888">${CAT_LABEL[p.category] ?? p.category}</span><br/><span style="font-size:11px">${p.element_reading ?? ''}</span>${p.interpretation ? `<br/><span style="font-size:11px;color:#666">${p.interpretation}</span>` : ''}<br/><span style="font-size:10px;color:#94a3b8">${p.source ?? ''}</span>`)
        .addTo(layer);
    });
  ...
  return (
    <div className="py-2 border-b border-slate-800/60 last:border-0">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {n.lat != null && <span className="text-[10px] text-emerald-300/80">◉ koord</span>}
      </div>
...
const Kalmar = () => {
  ...
  const byMatch = (m: string) => places.filter((p) => p.sol_match === m).sort((a, b) => a.name.localeCompare(b.name, 'sv'));
  const locality = byMatch('locality'), element = byMatch('element'), none = byMatch('none');
    ...
    <div className="min-h-screen viking-bg">
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <Card className="viking-card mb-4">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2 opacity-75">
              <strong>Guld</strong> = Hossmo husaby (koord: kyrkan, RAÄ Fornsök). <strong>Lila</strong> = övrigt koordinatsatt ortnamn.
              <strong> Blå</strong> = hamnen Kättilen. <strong>Metallfärg</strong> = myntfynd (find_place). Endast objekt med
              <em> belagd</em> koordinat ritas — {geocoded} av {places.length} ortnamn är geokodade hittills; resten listas nedan.
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="viking-card mb-4">
          ...
          <CardContent className="text-sm text-muted-foreground space-y-3">
            <p className="text-xs opacity-80">
              Uppslagna mot <strong>Svenskt ortnamnslexikon (SOL) 2003</strong>. Färgen visar hur säkert belägget är:
              grönt = SOL behandlar exakt orten, gult = SOL ger bara elementets betydelse (uppslaget avser en annan ort),
              grått = namnet finns inte i SOL (endast transparent ledanalys — hypotes, ej belagt). Inget är påhittat.
            </p>
            {locality.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-emerald-300 mb-1">SOL belägger orten ({locality.length})</div>
                {locality.map((n) => <NameRow key={n.id} n={n} />)}
              </div>
            )}
            {element.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-amber-300 mb-1">SOL belägger bara elementet ({element.length})</div>
                {element.map((n) => <NameRow key={n.id} n={n} />)}
              </div>
            )}
            {none.length > 0 && (
              <div>
                <div className="text-xs font-semibold text-slate-400 mb-1">Ej i SOL — endast ledanalys ({none.length})</div>
                {none.map((n) => <NameRow key={n.id} n={n} />)}
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="viking-card mb-4 border-amber-600/40">
          ...
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <p><strong className="text-foreground">Belagt:</strong> {locality.length} ortnamn behandlas direkt i SOL 2003 (bl.a. <em>Hossmo = *Husa + mo</em>, husaby-tolkningen). Hossmo rundkyrka (ca 1120) och hamnen Kättilen är källförda med koordinat (RAÄ Fornsök).</p>
            <p><strong className="text-amber-300">Tolkning, ej dom:</strong> att Hossmo-Rinkaby-noden var Möres maktcentrum <em>före</em> Kalmar är en välgrundad hypotes utifrån husaby-/rink-namnen — forskaren avgör. {element.length} namn har bara elementbetydelse i SOL (uppslaget avser annan ort); {none.length} saknas i SOL och står som ren ledanalys.</p>
            <p><strong className="text-foreground">Pending (nästa steg):</strong> geokoda de {places.length - geocoded} återstående ortnamnen mot RAÄ Fornsök/Lantmäteriet (koordinater får inte gissas), samt binda in blodbadet 1505 och Kalmar-/Öland-släkterna.</p>
          </CardContent>
        </Card>
