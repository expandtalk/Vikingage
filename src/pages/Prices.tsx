  ...
  return (
    <div className="min-h-screen viking-bg">
      ...
      <div className="container mx-auto px-4 py-6">
        <Tabs defaultValue="silver" className="max-w-4xl mx-auto">
          <TabsList className="grid w-full grid-cols-2 mb-4 max-w-xl mx-auto">
            <TabsTrigger value="silver" className="gap-1.5">
              <Scale className="h-4 w-4" /> {sv ? 'Vikingatida silver' : 'Viking silver'}
            </TabsTrigger>
            <TabsTrigger value="converter" className="gap-1.5">
              <Coins className="h-4 w-4" /> {sv ? 'Prisomvandlare' : 'Price Converter'}
            </TabsTrigger>
          </TabsList>

          {/* Flik 1: vikingatida vägd silverekonomi (hjälten) */}
          <TabsContent value="silver">
            <VikingSilverConverter />
          </TabsContent>

          {/* Flik 2: romersk tid — Diocletianus prisedikt (romerska importfynden) */}
          <TabsContent value="converter">
            <div className="bg-slate-800/90 border border-slate-700 rounded-lg p-6 mb-6">
              <h2 className="text-2xl font-bold text-orange-400 mb-3">
                {sv ? 'Diocletianus Prisförordning (301 e.Kr.)' : 'Diocletian Price Edict (301 CE)'}
              </h2>
              <p className="text-slate-300 mb-2">
                {sv
                  ? 'Kejsare Diocletianus regerade från 286 till 305 e.Kr. - långt innan vikingatiden (ca 793-1066). Prisförordningen från år 301 är ett av de viktigaste dokumenten för att förstå den senromerska ekonomin.'
