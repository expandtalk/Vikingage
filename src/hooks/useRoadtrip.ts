...
let state: State = { dest: null, route: null, corridor: [], status: 'idle', error: null };
const listeners = new Set<() => void>();
const emit = () => listeners.forEach((l) => l());
const subscribe = (l: () => void) => { listeners.add(l); return () => { listeners.delete(l); }; };

export const setRoadtripSearching = () => { state = { ...state, status: 'searching', error: null }; emit(); };
export const setRoadtripResult = (dest: RoadtripDest, route: RouteResult) => { state = { ...state, dest, route, status: 'done', error: null }; emit(); };
export const setRoadtripCorridor = (corridor: AlongRouteFeature[]) => { state = { ...state, corridor }; emit(); };
export const setRoadtripError = (error: string) => { state = { ...state, status: 'error', error }; emit(); };
export const clearRoadtrip = () => { state = { dest: null, route: null, corridor: [], status: 'idle', error: null }; emit(); };
