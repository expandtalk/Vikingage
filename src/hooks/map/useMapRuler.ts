...
export const useMapRuler = ({ map, isMapReady }: Props) => {
  ...
  useEffect(() => {
    ...
    if (pts.length === 2) {
      L.polyline([[pts[0].lat, pts[0].lng], [pts[1].lat, pts[1].lng]], { color: '#f59e0b', weight: 2, dashArray: '6 4' }).addTo(layer);
      const km = rulerKm(pts[0], pts[1]);
      const mid: [number, number] = [(pts[0].lat + pts[1].lat) / 2, (pts[0].lng + pts[1].lng) / 2];
      L.marker(mid, {
        icon: L.divIcon({
          className: 'ruler-label',
          html: `<div style="background:#78350f;color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;white-space:nowrap">${km.toFixed(1)} km</div>`,
