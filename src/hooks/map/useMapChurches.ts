...
const popupHtml = (r: any) => {
  ...
  const histBtn = r.id
    ? `<button type="button" onclick="window.__openChurchHistory&&window.__openChurchHistory('id:${r.id}')" style="margin-top:8px;padding:4px 8px;border:1px solid #b45309;border-radius:6px;background:transparent;color:#b45309;cursor:pointer;font-size:11px">Byggnadshistoria →</button>`
    : '';
  return `<div style="min-width:200px">
    <strong>${esc(r.name)}</strong>${ruin ? ' <span style="color:#6b7280">· ruin</span>' : ''}
    ${img}
    <div style="font-size:12px;color:#475569;line-height:1.5">${rows.join('<br/>')}</div>${attr}${histBtn}
  </div>`;
};
