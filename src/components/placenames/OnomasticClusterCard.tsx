...
// Två landskap: Öland (Daniel) och Ångermanland (Agneta Nyholms centralplatser som epicentra).
...
type Region = 'Öland' | 'Ångermanland';
interface Case {
  id: string; region: Region; label: string; lat: number; lng: number;
  elements: string[]; exclude: string | null; kind: 'positiv' | 'negativ';
  note: string;
}

// Ringradier per landskap: Ångermanland är vidsträckt/glest → större ringar än Öland.
const RINGS: Record<Region, number[]> = {
  'Öland': [2.5, 5, 7.5, 10, 15],
  'Ångermanland': [5, 10, 15, 20, 30],
};

// Kult-/maktuppsättningen Agneta använder (per led litet n → testas kombinerat).
const NORR_CULT = ['frö', 'sal', 'ross', 'hammar', 'hov', 'tor'];

const CASES: Case[] = [
  // ── Öland (Daniel) — pilotfallen ──
  { id: 'h1', region: 'Öland', kind: 'positiv', label: 'Sandby borg → -by (bebyggelse)', lat: 56.55253, lng: 16.63926, elements: ['by'], exclude: 'sandby',
  ...
  { id: 'h2', region: 'Öland', kind: 'positiv', label: 'Gråborg → -torp (röjningsskikt)', lat: 56.66642, lng: 16.60399, elements: ['torp'], exclude: 'graborg',
  ...
  { id: 'n1', region: 'Öland', kind: 'negativ', label: 'Ismantorps borg → -by (NEGATIV kontroll)', lat: 56.74544, lng: 16.64268, elements: ['by'], exclude: null,
  ...
  { id: 'n2', region: 'Öland', kind: 'negativ', label: 'Gärdslösa kyrka → -by (NEGATIV kontroll)', lat: 56.7935, lng: 16.73792, elements: ['by'], exclude: null,
  ...
  { id: 'a_harnosand', region: 'Ångermanland', kind: 'positiv', label: 'Härnösand–Säbrå → kult/makt-led', lat: 62.63226, lng: 17.93823, elements: NORR_CULT, exclude: null,
    note: 'Äldsta centralområdet; Norrlands första biskopssäte. Hov/Stav/Ed/Ross/Fröja-kluster.' },
  { id: 'a_neg', region: 'Ångermanland', kind: 'negativ', label: 'Nora → -sjö (NEGATIV kontroll)', lat: 62.87227, lng: 18.08104, elements: ['sjö'], exclude: null,
    note: 'Topografiskt naturled (-sjö), jämnt spritt. I norr duger inte kyrkor som nollkontroll (glest, sent kristnat) → topografin bär bruset. Väntat: ingen anrikning över bakgrund.' },
];

const R_PX = 120;

const OnomasticClusterCard: React.FC = () => {
  const [region, setRegion] = useState<Region>('Öland');
  const [caseId, setCaseId] = useState('h1');
  const [rings, setRings] = useState<Ring[]>([]);
  const [loading, setLoading] = useState(false);
  const c = CASES.find((x) => x.id === caseId) ?? CASES[0];
  const ringEdges = RINGS[c.region];
  ...
  useEffect(() => {
    ...
    const hit = CASES.find((x) => x.id === id);
    if (hit) { setRegion(hit.region); setCaseId(hit.id); }
  }, [caseSeq]);

  const pickRegion = (r: Region) => {
    setRegion(r);
    const first = CASES.find((x) => x.region === r);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await (supabase.rpc as unknown as (fn: string, args: Record<string, unknown>) => Promise<{ data: unknown }>)(
        ...
        { p_lat: c.lat, p_lng: c.lng, p_element_keys: c.elements, p_ring_edges_km: ringEdges, p_exclude_home: c.exclude, p_province: c.region },
      );
      if (!alive) return;
  ...
  const maxKm = ringEdges[ringEdges.length - 1];
  // Skarp kant = yttersta ring i den SAMMANHÄNGANDE över-bandet-sviten som börjar i CENTRUM.
  // (Bimodala profiler — en sekundär bump långt ut — är ingen skarp kant från epicentret. Är inte
  // ens innersta ringen över bandet finns ingen central kant → edgeR=null.)
  const ordered = [...rings].sort((a, b) => Number(a.ring_from_km) - Number(b.ring_from_km));
  let edgeR: number | null = null;
  for (const r of ordered) { if (r.over_band) edgeR = Number(r.ring_to_km); else break; }
  const anyOver = rings.some((r) => r.over_band);
  const underpowered = rings.some((r) => r.total_n > 0 && r.total_n < 12);

  const tier = (r: Ring): string => {
    if (r.over_band) return '#d4a63c';
    if (r.frac != null && r.frac > r.exp_frac) return '#8a7a5c';
    return '#33414d';
  };

  const regionCases = CASES.filter((x) => x.region === region);
  const bgNote = region === 'Öland'
    ? 'Öland: 395 ortnamn med geometri, 100% OSM/Wikidata utan primärbelägg → led-analys kräver SOL 2003 / Isof för verifierade äldsta belägg. Bakgrund = alla Ölands ortnamn.'
    : 'Ångermanland: gles, sent kristnad bygd — kult-/maktleden är fåtaliga (per led n≈1–9), därför testas de KOMBINERAT. Bakgrund = alla ångermanländska ortnamn; epicentra = Agneta Nyholms centralplatser (hypotes). Litet n → tolka med försiktighet.';
    ...
    <Card className="viking-card mb-4 border-gold/30">
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Testar om ett namnled <strong>anrikas</strong> kring ett förregistrerat epicentrum (fornborg, kyrka, centralplats),
          mätt <strong>mot bakgrunden</strong> (alla ortnamn i landskapet) ring för ring. Ett riktigt kluster har en{' '}
          <strong>skarp kant</strong>: inre ringar ligger signifikant över bandet, sedan faller profilen tillbaka.
        </p>

        {/* Landskaps-väljare */}
        <div className="flex gap-1.5">
          {(['Öland', 'Ångermanland'] as Region[]).map((r) => (
            <button key={r} onClick={() => pickRegion(r)}
              className={`rounded-md border px-2.5 py-1 text-xs ${region === r ? 'border-gold bg-gold/15 text-gold' : 'border-slate-600 text-slate-300 hover:border-gold/50'}`}>
              {r}
            </button>
          ))}
        </div>
        ...
        <select value={caseId} onChange={(e) => setCaseId(e.target.value)}
          ...
          {regionCases.map((x) => <option key={x.id} value={x.id}>{x.kind === 'negativ' ? '⊘ ' : '● '}{x.label}</option>)}
        </select>
        <p className="text-xs text-muted-foreground opacity-80">{c.note}</p>
        {region === 'Ångermanland' && (
          <p className="text-[11px] text-cyan-300/80">Epicentra: Agneta Nyholm (ortnamnskluster, Ångermanland) — hypotes.</p>
        )}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col items-center">
              <svg viewBox="0 0 260 260" className="w-full max-w-[260px]">
                ...
                {ringEdges.map((kmv) => (
                  <text key={kmv} x="130" y={130 - (kmv / maxKm) * R_PX + 10} textAnchor="middle" fontSize="8" fill="#94a3b8">{kmv} km</text>
                ))}
              </svg>
              <div className="text-center text-sm mt-1">
                {edgeR != null ? (
                  <span className="text-gold font-medium">Kluster med skarp kant vid ≈ {edgeR} km</span>
                ) : anyOver ? (
                  <span className="text-amber-300">Diffus/sekundär anrikning (ingen skarp kant från centrum)</span>
                ) : (
