...
// En kompakt "välj din lins"-launcher på förstasidan. Transparenta kort (som
// HeroStatsGrid); siffran är centrerad sist på kortet. Kort utan siffra läggs sist.
interface LauncherCard {
  ...
  dsv: string;
  den: string;
  // Räknare hämtas ur den delade stats-källan; utelämnas där siffra saknas/vilseleder.
...
const CARDS: LauncherCard[] = [
  ...
  // Utan siffra (siffran togs bort / saknas) — hamnar sist automatiskt vid rendering.
...
export const ViewLauncherGrid: React.FC<ViewLauncherGridProps> = ({ dbStats }) => {
  ...
  // Kort med siffra först, utan siffra sist (stabil ordning inom respektive grupp).
  ...
  const Card: React.FC<{ c: LauncherCard; n?: number }> = ({ c, n }) => (
    ...
    >
      ...
      <div className="mt-auto pt-2">
        {typeof n === 'number' ? (
          <span className="block text-xl font-bold text-white tabular-nums leading-none">{fmt(n)}</span>
        ) : (
