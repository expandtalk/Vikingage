// Steg 2: hämtar CC0-fornlämningar (RAÄ K-samsök) per typ, filtrerar till äkta
// lämningar med verifierad WGS84-koordinat, POSTar batchar till import-heritage.
// Samma verifieringsregel som vårdkasarna: bara gml:coordinates direkt ur RAÄ.
...
// term = fritextsök, label = raa_type att lagra, re = måste matcha itemLabel
const TYPES = [
  { term: 'dös', label: 'dös', re: /dös/i },
  { term: 'gånggrift', label: 'gånggrift', re: /gånggrift/i },
  { term: 'bildsten', label: 'bildsten', re: /bildsten/i },
  { term: 'skeppssättning', label: 'skeppssättning', re: /skeppss/i },
];
...
const fetchType = async (t) => {
  ...
  for (let start = 1; start <= MAX_PAGES * PER; start += PER) {
    ...
    for (const it of items) {
      const type = (it.match(/<pres:type[^>]*>([^<]*)</) || [])[1] || '';
      ...
      // Endast RAÄ Fornsök-LÄMNINGAR (ej museifoton/ritningar) med verifierad koord.
      if (!/\/raa\/lamning\//.test(uri)) continue;
      // Typordet ska förekomma i posten (lämningstyp) — annars är det en granne som råkar nämna ordet.
