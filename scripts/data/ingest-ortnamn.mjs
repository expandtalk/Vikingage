...
async function main() {
  ...
  console.log(`Exempelrad: name=${pick(sampleRow, NAME_KEYS)}, type=${pick(sampleRow, TYPE_KEYS)}, ext=${buildExtId(sampleRow)}`);
  ...
  const bboxClause = args.bbox
    ? `WHERE ST_Contains(ST_MakeEnvelope($6,$7,$8,$9,4326), p.g)` : ``;
  const sql =
    `INSERT INTO public.place_names (name, lat, lng, feature_type, external_id, source, source_license)
     SELECT u.name, ST_Y(p.g), ST_X(p.g), u.ft, u.ext, '${SOURCE}', '${LICENSE}'
     FROM unnest($1::text[], $2::text[], $3::text[], $4::text[]) AS u(name, wkbhex, ft, ext)
     CROSS JOIN LATERAL (SELECT ST_Centroid(ST_Transform(ST_GeomFromWKB(decode(u.wkbhex,'hex'), $5), 4326)) g) p
     ${bboxClause}
     ON CONFLICT (source, external_id)
     DO UPDATE SET name=EXCLUDED.name, lat=EXCLUDED.lat, lng=EXCLUDED.lng,
                   feature_type=EXCLUDED.feature_type, updated_at=now()`;

  let seen = 0, inserted = 0;
  let bName = [], bWkb = [], bFt = [], bExt = [];
  const flush = async () => {
    if (!bName.length) return;
    const params = [bName, bWkb, bFt, bExt, srsId];
  ...
  try {
    ...
    while (st.step()) {
      ...
      if (args.types && !args.types.has(ftVal)) continue; // typ-filter (--type)
