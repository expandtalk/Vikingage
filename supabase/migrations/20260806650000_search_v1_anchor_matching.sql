-- search_v1: ankrad matchning (word_similarity>=0.6 + label-anchor-multiplikator + golvtröskel
-- + utflykts-boost). Löser Köla/Höla på öland och Nås/Svenstorp på Stockholm. Repo-spegling. 2026-08-06.
  ...
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    f.score
      ...
      * least(1.5, 1 + 0.30 * coalesce(d.prominence, 0))
      -- LABEL-ANCHOR: exakt/prefix tungt, intern-substräng lätt, EJ i label (body/fuzzy) nästan noll.
      * (case
      ...
      -- ENTITETSTYP-VIKT (+ utflykter = kurerad besöksdata, boostas i Upptäck).
...
select entity_type, entity_id, signum, label,
  ...
  snippet, score
from deduped
where dup_rn = 1 and score >= 0.010          -- golvtröskel: kapar svansbruset
order by score desc
limit p_limit
$function$
