-- Lägg interpretation_claims (forskningstolkningar: läsordning m.m., med scholar/källa/status/note)
-- i get_inscription_page så inskriptssidan kan rendera dem. Bevarar allt övrigt oförändrat.
...
AS $function$
  select to_jsonb(x) from (
    select
      ...
      ,(select coalesce(jsonb_agg(jsonb_build_object(
                'part_key', ic.part_key, 'reading_translit', ic.reading_translit,
                'normalization', ic.normalization, 'translation', ic.translation,
                'scholar', ic.scholar_name, 'year', ic.year, 'source', ic.source,
                'status', ic.status, 'confidence', ic.confidence, 'note', ic.note)
                order by ic.confidence desc nulls last, ic.year desc nulls last), '[]'::jsonb)
       from public.interpretation_claim ic where ic.inscription_id = ri.id
      ) as interpretation_claims
