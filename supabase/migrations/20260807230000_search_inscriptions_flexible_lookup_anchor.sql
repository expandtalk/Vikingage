-- Ankare för explore-inskriftssöket + CompactSearchBox. Roten till "Karlevistenen → Sigtunadosan":
-- funktionen matchade substräng ÄVEN i fritext (translation/historical_context/scholarly_notes), så
-- ett namn matchade andra stenars ANTECKNINGAR som nämner det. Konsekvent regel med search_v2:
-- ENORDSSÖK (p_search_term utan mellanslag) = namn/plats-uppslag → matcha bara identitets-/geografifält;
-- fritext bara vid FLERORDS-/innehållssökning. Verifierat: "Karlevistenen" → bara DR 411 (ej Sigtunadosan).
...
DECLARE
  ...
  is_lookup boolean := (btrim(p_search_term) !~ '\s');
BEGIN
  IF LOWER(p_search_term) IN ('gotland', 'småland', 'uppland', 'östergötland', 'västergötland', 'sörmland', 'närke', 'västmanland') THEN
    ...
    landscape_signums := CASE LOWER(p_search_term)
      WHEN 'gotland' THEN ARRAY['G'] WHEN 'småland' THEN ARRAY['Sm', 'SM']
      WHEN 'uppland' THEN ARRAY['U'] WHEN 'östergötland' THEN ARRAY['Ög', 'ÖG']
      WHEN 'västergötland' THEN ARRAY['Vg', 'VG'] WHEN 'sörmland' THEN ARRAY['Sö', 'SÖ']
      WHEN 'närke' THEN ARRAY['Nä', 'NÄ'] WHEN 'västmanland' THEN ARRAY['Vs', 'VS']
      ELSE ARRAY[]::text[] END;
  END IF;

  RETURN QUERY
  SELECT
    ri.id, COALESCE(ri.primary_signum, ri.signum) as signum,
    ri.transliteration, ri.normalization, ri.translation_en, ri.translation_sv, ri.dating_text,
    ...
    ri.name, ri.name_en, ri.also_known_as, ri.alternative_signum, ri.primary_signum
  FROM runic_inscriptions ri
  WHERE
    CASE
      WHEN is_landscape_search THEN (
        EXISTS (SELECT 1 FROM unnest(landscape_signums) AS prefix
                WHERE COALESCE(ri.primary_signum, ri.signum) ~ ('^' || prefix || '\s*\d+'))
        OR LOWER(ri.landscape) LIKE LOWER('%' || p_search_term || '%')
      )
      ELSE (
        LOWER(COALESCE(ri.primary_signum, ri.signum)) LIKE LOWER('%' || p_search_term || '%') OR
        EXISTS (SELECT 1 FROM unnest(ri.alternative_signum) AS alt_sig WHERE LOWER(alt_sig) LIKE LOWER('%' || p_search_term || '%')) OR
        LOWER(ri.name) LIKE LOWER('%' || p_search_term || '%') OR
        LOWER(ri.name_en) LIKE LOWER('%' || p_search_term || '%') OR
        EXISTS (SELECT 1 FROM unnest(ri.also_known_as) AS alt_name WHERE LOWER(alt_name) LIKE LOWER('%' || p_search_term || '%')) OR
  ...
  ORDER BY
    CASE
      ...
      ELSE 5
    END,
    CASE WHEN ri.name IS NOT NULL AND ri.name <> '' THEN 0 ELSE 1 END,
    (regexp_match(COALESCE(ri.primary_signum, ri.signum), '^([A-ZÄÖÅ]+)'))[1],
    COALESCE((regexp_match(COALESCE(ri.primary_signum, ri.signum), '\s*(\d+)'))[1]::integer, 0) ASC;
END;
$function$;
