-- Nordisk för-foldning: danskt/norskt æ→ä och ø→ö INNAN unaccent, så vokal-symmetrin blir komplett.
-- Utan detta: unaccent(æ)=ae men unaccent(ä)=a → Næs (dansk) matchar inte Näs (svensk). Med detta:
-- æ→ä→a och ø→ö→o → Næs=Näs och øland=öland. å delas redan av alla nordiska språk.
-- Verifierat: Næs=Näs, øland=Öland, Ærø=Ärö; Straße→strasse & Þingvellir→thingvellir orörda;
-- Kärna fortf. borta för "kalmar", "gamla uppsala"→Uppsala kvar.
-- OBS: isländskt/fornnordiskt æ i namn (Ægir) foldas nu till a i st.f. ae — acceptabelt för
-- skandinavisk ortnamnssökning; ASCII "ae"-expansion (Aeroe) hör ändå till alias-data (name_authority).
-- search_fold är STABLE (unaccent beror på dictionary). För framtida funktionellt trgm-index på
-- unaccent(label) krävs en IMMUTABLE-variant — separat, större ändring.
-- Applicerad i prod via MCP; denna fil = repo-spegling. 2026-08-07.
CREATE OR REPLACE FUNCTION public.search_fold(t text)
 RETURNS text LANGUAGE sql STABLE SET search_path TO 'public', 'extensions'
AS $function$
  select extensions.unaccent(translate(lower(coalesce(t, '')), 'øæ', 'öä'))
$function$;
...
trgm as (
  select entity_type, entity_id, row_number() over (order by sim desc) as rnk from (
    ...
    where length(q.raw) >= 2
      and (d.label ilike q.raw || '%' or d.label ilike '%' || q.raw || '%' or word_similarity(q.raw, d.label) >= 0.60)
...
scored as (
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    -- PLATS-ANKARE (universellt, nordisk-fold via search_fold): foldad delsträng på hela frasen,
    -- ELLER minst ett betydande ord (≥3 tecken), ELLER foldad trgm-likhet ≥0.55.
    ( search_fold(d.label) like '%' || search_fold(q.raw) || '%'
      ...
      or exists (
           ...
           where length(tok) >= 3
             and search_fold(d.label) like '%' || tok || '%'
         )
    ) as label_has_q,
    f.score
      * (case
      ...
      * (case when lower(d.label) = lower(q.raw) then 1.0
              ...
              when position(lower(q.raw) in lower(d.label)) > 0 then 0.25 else 0.28 end)
