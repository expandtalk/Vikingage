-- Near me: bredda runstenens vardagsnamn till att även läsa `name`-kolumnen (där t.ex.
-- "Norslundastenen"/"Ardrestenarna"/"Sjonhemsmonumentet" bor — U 419 saknar also_known_as).
-- `name` innehåller lite platsbrus ("Örebro", "Hagia Sophia") → använd bara när den läser
-- som ett stennamn (sten|häll|ristning|monument|bleck), annars also_known_as-match, annars signum.
-- Gäller nearby_features + nearby_features_ranked. Signaturer oförändrade → CREATE OR REPLACE.
-- (Applicerad via MCP apply_migration; denna fil = källa för reproducerbarhet.)
