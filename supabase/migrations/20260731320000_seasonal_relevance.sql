-- Nearby-rank: säsongslager. Relevansen skiftar med månad/tradition (väder + sed): en källa
-- är het vid midsommar, en skeppssättning vid vårdagjämning, en kultplats vid jul. Säsong är
-- INTE en statisk per-plats-signal utan (plats-attribut × säsong) → egen matris-tabell, och
-- appliceras via RPC-param p_season (klienten vet månaden; server förblir objektiv utan param).
CREATE TABLE IF NOT EXISTS seasonal_relevance (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  season text NOT NULL,        -- jul | nyar | varsdagjamning | var | pask | sommar | midsommar | host
  match_kind text NOT NULL,    -- feature_type | raa_type
  match_value text NOT NULL,
  weight numeric NOT NULL DEFAULT 0.2,
  note text,
  UNIQUE (season, match_kind, match_value)
);
ALTER TABLE seasonal_relevance ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS seasonal_relevance_read ON seasonal_relevance;
CREATE POLICY seasonal_relevance_read ON seasonal_relevance FOR SELECT USING (true);

INSERT INTO seasonal_relevance (season, match_kind, match_value, weight, note) VALUES
  ('jul','feature_type','cult_site',0.30,'midvinterblót'),
  ('jul','feature_type','church',0.20,'julgudstjänst'),
  ('nyar','feature_type','cult_site',0.25,'midvinter/årsskifte'),
  ('varsdagjamning','raa_type','skeppssättning',0.35,'sol-riktade monument (jfr Ales stenar)'),
  ('varsdagjamning','raa_type','hällristning',0.30,'sol/rituell'),
  ('var','feature_type','runestone',0.15,'tillgängligt utomhus'),
  ('pask','feature_type','church',0.30,'påskgudstjänst'),
  ('pask','raa_type','Källa med tradition',0.25,'heliga källor'),
  ('sommar','feature_type','runestone',0.15,'utomhus/tillgängligt'),
  ('sommar','feature_type','heritage',0.10,'utomhus brett'),
  ('midsommar','feature_type','cult_site',0.35,'fruktbarhet/samling'),
  ('midsommar','raa_type','Källa med tradition',0.30,'midsommarkällor'),
  ('host','feature_type','estate',0.20,'skörd/gård'),
  ('host','feature_type','cult_site',0.20,'disablót')
ON CONFLICT (season, match_kind, match_value) DO UPDATE SET weight = excluded.weight, note = excluded.note;

-- RPC v3: valfri p_season → säsongsboost (matris) ovanpå Fas 1/2. Utan p_season = oförändrat.
