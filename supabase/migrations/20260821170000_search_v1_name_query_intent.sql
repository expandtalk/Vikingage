-- B2 (Daniel 2026-08-21): namn-/efternamnsfrågor rankade platser/kyrkor/källor ÖVER personer
-- (Kalle→place_name först, Larsson→bara böcker/källor, Sven/Erik→source inblandat). Rotorsak: type-
-- multiplikatorn i search_v1 boostar landscape/city/parish men ger person bara ×1.0 (else-grenen).
--
-- Fix: ett AVSIKTSLAGER. `is_name` = frågan matchar EXAKT en canonical i name_authority ELLER ett
-- efternamn i surname_stats. ENDAST då byts type-vikterna: person/saint/carver upp, place_name/place/
-- parish/hundred/landscape/source ned. Icke-namn-frågor (skärkind, oskarshamn, röksten …) tar else-
-- grenen med EXAKT dagens vikter → result-ekvivalent. Allt annat i funktionen är oförändrat mot
-- 20260820230000 (trgm-operatorform, geom-index m.m.).
...
with q as (
  select trim(p_q) as raw, lower(replace(trim(p_q), ' ', '')) as qnorm,
         ...
         (exists (select 1 from name_authority na where lower(na.canonical) = lower(trim(p_q)))
          or exists (select 1 from surname_stats ss where lower(ss.name) = lower(trim(p_q)))) as is_name
),
exact_sig as (
...
scored as (
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    f.score
      ...
      * (case
          when q.is_name then
            -- AVSIKTSLAGER: frågan ÄR ett känt för-/efternamn → personer/helgon/ristare främst,
            -- orter/socknar/källor demotas (de är sällan det man menar när man söker ett namn).
            case d.entity_type when 'person' then 1.8 when 'saint' then 1.8 when 'carver' then 1.3
                 when 'place_name' then 0.30 when 'place' then 0.30 when 'parish' then 0.5
                 when 'hundred' then 0.5 when 'landscape' then 0.6 when 'source' then 0.4 else 1.0 end
          else
