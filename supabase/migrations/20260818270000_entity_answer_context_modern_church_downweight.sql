-- Nya kyrkor lägre vikt: kyrko-landmärken som koordinatmatchar (<150 m) en MODERN kyrka
-- (ecclesiastical_sites byggd >= 1550) exkluderas ur svarets landmärkesbilder. T.ex. Nådens kapell
-- (Färjestaden, 1976). Medeltida kyrkor (<1550) behålls. Bygger på landskaps-fixen (20260818260000).
...
sites AS (
  SELECT h.id, h.name, h.raa_type, h.parish, left(h.description, 240) AS descr, h.lat, h.lng,
    heritage_significance(h.raa_type, sd.prominence, sd.popularity, h.place_slug IS NOT NULL)
      / (1 + (ST_Distance(ST_SetSRID(ST_MakePoint(h.lng,h.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography) / 2000.0)) AS score
  ...
  WHERE h.lat IS NOT NULL AND ctr.lat IS NOT NULL AND NOT EXISTS (SELECT 1 FROM theme)
    AND ST_DWithin(ST_SetSRID(ST_MakePoint(h.lng,h.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography, 12000)
...
churches AS (
  ...
  WHERE e.lat IS NOT NULL AND ctr.lat IS NOT NULL AND NOT EXISTS (SELECT 1 FROM theme)
    AND ST_DWithin(ST_SetSRID(ST_MakePoint(e.lng,e.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography, 12000)
    AND ((ctr.lat BETWEEN 56.19 AND 57.37 AND ctr.lng BETWEEN 16.40 AND 17.13)
  ...
  ORDER BY (CASE
      ...
      ELSE 1 END),
    ST_Distance(ST_SetSRID(ST_MakePoint(e.lng,e.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography) LIMIT 12
),
museums AS (
  ...
  WHERE mu.lat IS NOT NULL AND ctr.lat IS NOT NULL AND NOT EXISTS (SELECT 1 FROM theme)
    AND ST_DWithin(ST_SetSRID(ST_MakePoint(mu.lng,mu.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography, 15000)
  ORDER BY ST_Distance(ST_SetSRID(ST_MakePoint(mu.lng,mu.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography) LIMIT 12
),
wrecks AS (
...
crossings AS (
  ...
  WHERE c.lat IS NOT NULL AND ctr.lat IS NOT NULL AND NOT EXISTS (SELECT 1 FROM theme)
    AND ST_DWithin(ST_SetSRID(ST_MakePoint(c.lng,c.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography, 25000)
  ORDER BY ST_Distance(ST_SetSRID(ST_MakePoint(c.lng,c.lat),4326)::geography, ST_SetSRID(ST_MakePoint(ctr.lng,ctr.lat),4326)::geography) LIMIT 10
),
img AS (
