-- SKALNING: buntar de per-sökning-lookups som AnswerContext idag gör som separata anrop (person, namn,
-- belägg, socken, jordebok, charters, fornvännen, FAQ) till EN server-side round-trip. ~8 anrop → 1.
-- Alias:ade så nyckel/fält matchar exakt de shapes frontend redan konsumerar (minimal omkoppling).
-- STABLE, ingen skrivning. Faller tillbaka gracefully (null/[]/{}) per nyckel.
