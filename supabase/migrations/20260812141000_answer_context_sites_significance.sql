-- Närhets-motorn: entity_answer_context.sites rankas nu på heritage_significance × närhet i st.f.
-- prominence>0-golvet. Generiskt skräp (namnlöst/vägmärke/fornlämningsliknande) uteslutet; radie
-- 25→15 km (bygde-skala). Lidingö: 1→10 lokala sites. Kalmar: oförändrad. Se plan
-- docs/superpowers/plans/2026-08-12-narhets-motor-plan.md. Endast sites-CTE + dess agg ändrade.
-- (Full funktionskropp — CREATE OR REPLACE; identisk med crossings-versionen utom sites.)
...
hsite AS (
  ...
  ORDER BY coalesce(sd.prominence,0) DESC, h.name LIMIT 1
),
pick AS (
