-- #3 Kalmarsund-blödning + #4 moderna kyrkor i notable sites.
-- sites: brantare avstånds-decay (/4000→/2000) + radie 15km→12km → Öland-orter drar ej in Kalmar
--   tvärs sundet. Uteslut heritage-kyrkor som REDAN täcks av det daterade Churches-lagret (<400 m)
--   → notable sites domineras ej av kyrkor, moderna kyrkor (Vasakyrkan, som ligger i ecclesiastical_sites)
--   faller bort; RAÄ-kyrkoruiner som saknas i kyrkolagret behålls.
-- churches: radie 20km→12km + medeltid-först-ordning (daterade gamla kyrkor före moderna).
...
sites AS (
  ...
  WHERE h.lat IS NOT NULL AND ctr.lat IS NOT NULL AND NOT EXISTS (SELECT 1 FROM theme)
    ...
    -- Aktiva kyrkor (raa_type 'kyrka', t.ex. Vasakyrkan/Domkyrkan) bor i det daterade Churches-lagret →
    -- uteslut ur notable sites (domineras annars av kyrkor). Kyrkoruiner/kyrkogårdar behålls (notabla).
    AND h.raa_type NOT ILIKE '%kyrka%'
...
SELECT jsonb_build_object(
  ...
  'sites', (SELECT coalesce(jsonb_agg(jsonb_build_object('id',id,'name',name,'type',raa_type,'lat',lat,'lng',lng) ORDER BY score DESC, name),'[]'::jsonb) FROM sites),
