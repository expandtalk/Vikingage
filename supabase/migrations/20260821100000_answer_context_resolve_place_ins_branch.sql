-- Fix #1 (final link): entity_answer_context resolved a center via resolve_place for a clicked
-- ortnamn (e.g. "Skärkinds gamla kyrka") but the ins CTE (nearby runestones) had no branch for a
-- resolve_place-derived center -> fell to name-matching -> 0 inscriptions (Ög 171/172 not shown).
-- Adds an rp CTE (resolve_place once, gated below page/hit/hsite) + an ins radius branch using rp,
-- and routes ctr through rp (one resolve_place call instead of two). Surfaces the nearby runestones.
...
rp AS (
  -- resolved center for a clicked place/ortnamn whose name isn't page/hit/hsite (e.g. "X gamla kyrka");
  -- resolve_place also normalizes "gamla|nya kyrka" -> "kyrka" so the demolished-church ortnamn lands
  -- on its sibling church's real coordinate. Gated so earlier, more specific matches win.
  SELECT lat, lng FROM resolve_place(p_name)
  WHERE NOT EXISTS (SELECT 1 FROM page) AND NOT EXISTS (SELECT 1 FROM theme)
    AND NOT EXISTS (SELECT 1 FROM hit) AND NOT EXISTS (SELECT 1 FROM hsite)
  LIMIT 1
),
ins AS (
  ...
  WHERE r.coordinates IS NOT NULL AND (
    CASE
      ...
      WHEN EXISTS (SELECT 1 FROM rp) THEN ST_DWithin(ST_SetSRID(ST_MakePoint(r.coordinates[0], r.coordinates[1]),4326)::geography, ST_SetSRID(ST_MakePoint((SELECT lng FROM rp),(SELECT lat FROM rp)),4326)::geography, 15000)
...
ctr AS (
  SELECT coalesce(CASE WHEN EXISTS(SELECT 1 FROM page) THEN (SELECT lat FROM page)
              ...
              WHEN NOT EXISTS(SELECT 1 FROM theme) AND (SELECT lat FROM rp) IS NOT NULL THEN (SELECT lat FROM rp) WHEN NOT EXISTS(SELECT 1 FROM theme) THEN (SELECT avg(lat) FROM ins) END) AS lat,
         coalesce(CASE WHEN EXISTS(SELECT 1 FROM page) THEN (SELECT lng FROM page)
              ...
              WHEN NOT EXISTS(SELECT 1 FROM theme) AND (SELECT lng FROM rp) IS NOT NULL THEN (SELECT lng FROM rp) WHEN NOT EXISTS(SELECT 1 FROM theme) THEN (SELECT avg(lng) FROM ins) END) AS lng
),
sites AS (
