-- nearby_features v2: lägg till hela heritage_sites-korpusen (gravar/rösen/vrak/fossil åker…)
-- + maritime_nodes, så Near me inte bara visar kyrkor. Heritage prefiltreras spatialt (gist-index).
-- Kyrkor exkluderas ur heritage-grenen (kommer redan via 'church'). Signaturen oförändrad.
  ...
  WITH src AS (
    ...
    SELECT 'heritage', h.id::text,
     ...
     WHERE h.geom IS NOT NULL
       AND h.raa_type NOT ILIKE '%kyrk%'
       AND ST_DWithin(h.geom, ST_SetSRID(ST_MakePoint(p_lng, p_lat),4326)::geography, p_radius_km*1000)
    UNION ALL
    SELECT 'maritime_node', m.id::text, m.name, m.lat, m.lng
      FROM maritime_nodes m WHERE m.lat IS NOT NULL AND m.lng IS NOT NULL
  ),
  dist AS (
    SELECT feature_type, feature_id, label, lat, lng,
           2 * 6371 * asin(sqrt(power(sin(radians(lat - p_lat)/2),2) +
             cos(radians(p_lat))*cos(radians(lat))*power(sin(radians(lng - p_lng)/2),2))) AS distance_km
      FROM src
  )
