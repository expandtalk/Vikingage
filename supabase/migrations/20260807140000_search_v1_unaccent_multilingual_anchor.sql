-- Plats-ankaret universellt över språk: byt handskriven svensk translate-fold mot unaccent.
-- Verifierat mot korpusens tecken: de ß→ss, da/no æ→ae + ø→o, is þ→th + ð→d, sv åäö.
-- Per-token + accent-okänsligt: en GEO-träff (parish/place_name/place/hundred/landscape/city)
-- behålls om den foldade labeln innehåller HELA frasen, ELLER minst ett betydande ord (≥3 tecken),
-- ELLER foldad trgm-likhet ≥0.55. Kör bara på ≤20 kandidater/typ → ingen prestandakostnad.
-- Ersätter 20260807120000 (translate/position-varianten, för trubbig: tappade flerord + diakriter).
-- GRÄNS: unaccent foldar bara diakriter. Tysk ue/oe-expansion (Luebeck≠Lubeck) och exonymer
-- (Gothenburg/Göteborg, Haithabu/Hedeby) hör till alias-data (name_authority/also_known_as),
-- INTE hit — vi gissar aldrig fram namnlikhet.
-- Verifierat: Kärna borta för "kalmar", Kalmar kvar, "gamla uppsala"→Uppsala kvar, "lübeck"→Lübeck kvar.
...
scored as (
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    ( extensions.unaccent(lower(d.label)) like '%' || extensions.unaccent(lower(q.raw)) || '%'
      or word_similarity(extensions.unaccent(lower(q.raw)), extensions.unaccent(lower(d.label))) >= 0.55
      or exists (
           select 1 from regexp_split_to_table(extensions.unaccent(lower(q.raw)), '\s+') tok
           where length(tok) >= 3
             and extensions.unaccent(lower(d.label)) like '%' || tok || '%'
         )
    ) as label_has_q,
    f.score
      * (case
