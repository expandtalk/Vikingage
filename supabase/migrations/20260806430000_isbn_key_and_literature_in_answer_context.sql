-- STEG 0 (bok-kommers): ISBN som förstklassig unik nyckel + surfa documents-länkad litteratur
-- i söksvaret. Metadata/nyckel — ingen prosa. ISBN-10 normaliserade till ISBN-13.
-- Applicerad i prod via MCP (denna fil = repo-spegling). 2026-08-06.

ALTER TABLE public.historical_sources ADD COLUMN IF NOT EXISTS isbn text;
DO $$ BEGIN
  ALTER TABLE public.historical_sources ADD CONSTRAINT historical_sources_isbn_format
    CHECK (isbn IS NULL OR isbn ~ '^[0-9]{13}$');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
CREATE UNIQUE INDEX IF NOT EXISTS historical_sources_isbn_uidx
  ON public.historical_sources (isbn) WHERE isbn IS NOT NULL;

UPDATE public.historical_sources SET isbn = '9789151842035' WHERE title = 'Arvet efter Gustav Vasa' AND isbn IS NULL;
UPDATE public.historical_sources SET isbn = '9789151842172' WHERE title = 'Kalmarunionens tid – från drottning Margareta till Kristian II' AND isbn IS NULL;
UPDATE public.historical_sources SET isbn = '9789151839042' WHERE title = 'Gustav Vasa – landsfader eller tyrann?' AND isbn IS NULL;
...
lit AS (
  -- Böcker/källor som uttryckligen documents-länkats till en entitet vars namn matchar sökningen
  -- (t.ex. "Gustav Vasa" → hans böcker). ISBN följer med → "Hitta boken"-länk i klienten.
