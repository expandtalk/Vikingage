-- search_v1: kyrko-åldersvikt via ecclesiastical_sites.heritage_site_id + strukturell regel
-- (kyrka ej i historiska registret = modern → ×0.10). Applicerad i prod via MCP (repo-spegling). 2026-08-06.
...
select * from (
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    f.score * (
      case
        -- Odaterbar modern frikyrka → nära noll.
        when d.label ~* '(immanuel|filadelfia|smyrna|betlehemsk|elimkyrk|pingstkyrk|missionskyrk|baptistkyrk|metodistkyrk|sjömanskyrk|korskyrk|citykyrk|allianskyrk|frälsningsarm|adventkyrk|equmenia|saronkyrk|katolska kyrkan)'
          then 0.05
        -- Kyrkobyggnad som INTE finns i det historiska sockenkyrko-registret → modern/icke-socken.
        when d.entity_type = 'heritage_site' and d.label ~* 'kyrkan?\M' and ec.heritage_site_id is null
          then 0.10
        -- Historisk sockenkyrka: åldersgradient ur registret (odaterad → faller till neutral 1.0).
