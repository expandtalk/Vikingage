-- Fix: get_inscription_page visade bara media_type='image' → de 372 historiska avbildningarna
-- (media_type='teckning', Peringskiöld/Hadorph/Bautil) doldes på inskriftssidan (t.ex. Sö 222).
-- Ändrat till media_type in ('image','teckning'). URL-extensionsfiltret (pub.raa.se/visning +
-- wikimedia) släpper redan igenom teckningarnas URL:er. Övrig funktionskropp oförändrad.
  ...
  select to_jsonb(x) from (
    select
      ...
      (select coalesce(jsonb_agg(jsonb_build_object(
                'url', m.media_url, 'description', m.description,
