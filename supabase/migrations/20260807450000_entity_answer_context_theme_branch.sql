-- Temagren i svarskontexten (pilot). En tematisk sökning ("poesi", "diktmått", "bildsten" …) resolvar
-- till ett tema och visar temats länkade runstenar + deras bilder i stället för en geografisk radie.
--
-- Precedens: content_page (kurerad plats) > landskap/socken/heritage-plats > TEMA > enskild sten (hit).
-- Ett tema får ALDRIG kapa en plats — därför aktiveras temagrenen bara om p_name inte matchar något
-- platsnamn (socken/location/parish/landscape i runinskrifter eller heritage). "Gotland" är landskap
-- men även nyckelord i "Gotländska bildstenar" → utan platsvakten kapades landskapssökningen.
--
-- GATE: temat aktiveras bara om det har >= 8 BILD-backade inskriftslänkar → tomma teman (t.ex. "Skepp
-- & hav" 0 länkar, "Guld" 0 bild-backade) läcker aldrig ut som blank tavla, utan faller tillbaka på
-- lexikal sökning. Tröskeln är självunderhållande: när KG-backfill fyller theme_links tänds teman av
-- sig själva. Additivt: ny nyckel 'theme'; alla befintliga nycklar (inkl. 'sites') behålls oförändrade.
...
theme AS (
  ...
  WHERE NOT EXISTS (SELECT 1 FROM page)
    -- plats vinner: aktivera inte tema om termen är ett känt platsnamn
    AND NOT EXISTS (
