-- search_v1: query-shape-oberoende demotering — tomhetsstraff (×0.15 innehållslösa platsnoder) +
-- exakt-namnboost (×2, slår straffet). Repo-spegling. 2026-08-06.
...
scored as (
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    f.score
      ...
      * (case
          when lower(d.label) = lower(q.raw) then 1.0
      ...
      * (case d.entity_type when 'landscape' then 1.6 when 'excursion' then 1.4 when 'city' then 1.3
      ...
      -- (a) TOMHETSSTRAFF: innehållslösa platsnoder (ej exakt namn) sjunker.
      * (case when d.entity_type in ('place_name','place')
                   and coalesce(d.popularity,0) = 0 and coalesce(d.prominence,0) = 0
                   and lower(d.label) <> lower(q.raw) then 0.15 else 1.0 end)
      -- (b) EXAKT-NAMNBOOST: slår alltid straffet (0.15) → exakt namn vinner.
