-- search_v1 steg 1: bounded popularitets-/prominence-boost (score = relevans × kyrko-vikt × popularitet × prominence).
...
select * from (
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    f.score
      ...
      * least(2.0, 1 + 0.15 * ln(1 + coalesce(d.popularity, 0)))          -- Wikipedia-intresse/sökvolym
      * least(1.5, 1 + 0.30 * coalesce(d.prominence, 0)) as score          -- kvalitet/vikt
  from diversified f
