-- Ordlista (glossary) — fas 1: mekanism + [REPO]-grundad seed. Människa-i-loopen: bara termer vars
-- definition är grundad i vår egen kanoniska kod/metodik (license='egen', verified=true). Källkänsliga
-- termer (etymologi/numismatik: hacksilver, attung m.fl.) seedas EJ här — de kräver verifierare + källa.
-- Ref: scratch-ordlista-utredning.md, [[search-query-understanding-benchmark]], [[search-index-architecture]].
...
create table if not exists public.glossary (
  ...
  register      text not null default 'domain',   -- 'domain' | 'archaic_word'
  source        text,
  source_url    text,
  license       text not null default 'unverified', -- 'PD' | 'CC-BY' | 'CC0' | 'egen' | 'unverified'
  epistemic     text not null default 'belagt',    -- 'belagt' | 'hypotes' | 'omtvistat' | 'obelagt'
...
create index if not exists glossary_term_trgm on public.glossary using gin (term gin_trgm_ops);

alter table public.glossary enable row level security;
do $$ begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='glossary' and policyname='glossary_public_read') then
    create policy glossary_public_read on public.glossary for select
      using (verified = true and license in ('PD','CC-BY','CC0','egen') and not exclude_from_search);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='glossary' and policyname='glossary_admin_all') then
    create policy glossary_admin_all on public.glossary for all
      using (public.is_admin()) with check (public.is_admin());
  end if;
end $$;

-- [REPO]-seed (6 termer). Definitioner grundade i repot; epistemic ärligt satt (teofort = hypotes).
insert into public.glossary (slug, term, short_def, definition, category, register, source, source_url, license, epistemic, see_also, lang, verified) values
('graslunds-ornamenttypologi', 'Gräslunds ornamenttypologi',
 'Stilkronologi för runstenars ornamentik (Pr1–Pr5, Fp, RAK) som ger ett mjukt dateringskuvert, inte ett exakt årtal.',
 'Anna-Lena Gräslunds typologi ordnar de sena runstenarnas ornamentik i stilfaser. I plattformens kalibrering mot Källström 2007 (Mästare och minnesmärken, s. 65–75) är Pr4 (ca 1070–1100) och Pr5 (ca 1100–1130) säkrast arkeologiskt ankrade, Pr3 medel, medan Fp/Pr1/Pr2 och RAK (ca 980–1075) är bredare och tvetydigare. Typologin daterar stilen, inte nödvändigtvis ristningens exakta år.',
 'runology', 'domain', 'Egen kalibrering (graslundPhases.ts) mot Källström 2007', '/sv/vetenskapsmetodik', 'egen', 'belagt', '{ristarattribution,signum}', 'sv', true),
('teofort-ortnamn', 'Teofort ortnamn',
 'Ortnamn som antas innehålla ett gudanamn (t.ex. Torslunda, Odensvi) — härledningen är en tolkning, inte ett fastställt faktum.',
 'Teofora ortnamn tolkas som möjliga spår av förkristen kult. I plattformens ortnamnsmodell (placeNameElements.ts) bär sakrala led en konfidensgrad och många är märkta som omtvistade: ett gudanamnsled är en hypotes om namnets ursprung, inte ett belägg för kult på platsen.',
 'onomastics', 'domain', 'placeNameElements.ts (sakrala lagret)', null, 'egen', 'hypotes', '{namnled,bebyggelsenamn}', 'sv', true),
('namnled', 'Namnled (för- och efterled)',
 'Ett ortnamns betydelsebärande beståndsdelar; efterledet anger oftast typ (-by, -hem), förledet en bestämning.',
 'Ortnamn analyseras i led. Efterledet klassar namntypen (bebyggelse, natur), förledet specificerar. I plattformens modell har varje led en gränsregel (förled/efterled/självständigt) som styr hur det får matchas.',
 'onomastics', 'domain', 'placeNameElements.ts', null, 'egen', 'belagt', '{aldsta-belagg,bebyggelsenamn}', 'sv', true),
('aldsta-belagg', 'Äldsta belägg',
 'Den tidigaste kända skriftliga formen av ett namn — belägg-åldern är inte samma sak som namnets ålder.',
 'Äldsta belägg är det tidigaste bevarade skriftliga omnämnandet. Ett namn kan vara långt äldre än sitt äldsta belägg; belägg-ålder och namn-ålder hålls därför isär (tvåaxlig redovisning).',
 'onomastics', 'domain', 'place_names.earliest_attestation_year', null, 'egen', 'belagt', '{namnled}', 'sv', true),
('bebyggelsenamn', 'Bebyggelsenamn / kontrollelement',
 'Namntyper som -by, -sta, -inge, -hem vilka daterar bebyggelseskikt och används som neutrala kontrollelement — inte som sakrala signaler.',
 'Bebyggelsenamnsled markerar sig i plattformen som kontrollelement (isControl): de daterar bosättningsskikt men bär ingen kultsignal, och används som baslinje mot vilken sakrala tolkningar prövas.',
 'onomastics', 'domain', 'placeNameElements.ts (isControl)', null, 'egen', 'belagt', '{namnled,teofort-ortnamn}', 'sv', true),
('observation-vs-tolkning', 'Observation vs tolkning',
 'Den metodologiska grundskillnaden mellan det som faktiskt iakttas och den uttydning man lägger på iakttagelsen.',
 'Kärnprincip i plattformens källkritik: observationen (det mätbara, iakttagbara) skiljs från tolkningen (uttydningen). Att blanda ihop dem är en vanlig felkälla — en plausibel tolkning presenteras då som om den vore en observation.',
 'chronology', 'domain', '/sv/vetenskapsmetodik', '/sv/vetenskapsmetodik', 'egen', 'belagt', '{}', 'sv', true)
on conflict (slug) do update set
  term=excluded.term, short_def=excluded.short_def, definition=excluded.definition, category=excluded.category,
  source=excluded.source, source_url=excluded.source_url, license=excluded.license, epistemic=excluded.epistemic,
  see_also=excluded.see_also, verified=excluded.verified, updated_at=now();


-- ==== rebuild_search_document_x (LIVE def från prod + glossary_term-block injicerat) ====
...
begin
  ...
  if p_type is null or p_type = 'glossary_term' then
    insert into search_document (entity_type, entity_id, signum, label, sublabel, body_sv, body_en, popularity)
    select 'glossary_term', g.id, '/sv/ordlista/'||g.slug, g.term,
      concat_ws(' · ', 'fackterm', g.category),
      concat_ws(' ', g.term, g.short_def, coalesce(g.definition,''), array_to_string(g.see_also,' ')),
      case when g.lang='en' then concat_ws(' ', g.term, g.short_def) else null end,
      1
    from glossary g
    where g.verified = true and not g.exclude_from_search and g.license <> 'unverified'
...
-- Reindexera bara ordlistan (wrappern deletar entity_type=glossary_term och anropar _x).
select public.rebuild_search_document('glossary_term');
