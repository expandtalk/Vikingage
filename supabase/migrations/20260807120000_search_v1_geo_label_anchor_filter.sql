-- Kärna/Kållands-Åsaka dök upp i SOCKNAR-gruppen för "Kalmar" (Daniels skärmdump): plats-typer matchade
-- svagt på delsträngar i beskrivningstext (fts/trgm) och överlevde i den låga svansen (rank 61–120).
-- REGEL (Daniel): en plats-träff MÅSTE innehålla söktexten i namnet — annars är det inte den platsen.
-- Uteslut GEO-typer (parish/place_name/place/hundred/landscape/city) vars label inte innehåller queryn.
-- Verifierat: search_v1('kalmar',120) GEO-grupper = bara Kalmar/Kalmar slott/Kalmar kyrka; Kärna borta.
...
scored as (
  select d.entity_type, d.entity_id, d.signum, d.label, d.sublabel,
    ...
    (position(lower(q.raw) in lower(d.label)) > 0) as label_has_q,
    f.score
      * (case
...
deduped as (
  ...
  from scored
  -- PLATS-ANKARE: en socken/ortnamn/plats/härad/landskap/stad måste innehålla söktexten i namnet.
