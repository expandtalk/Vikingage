-- Nearby-rank v1: avstånd är EN vikt, inte hela ordningen. Objektiva signaler server-side;
-- personliga (intresse, släkt/GEDCOM, vänner, tid-events) läggs på KLIENTSIDAN (privacy — släkt
-- är klientsidig) ovanpå score. Auktoritet = grad i VÅR relationship-graf (vår "PageRank", ej
-- Googles patenterade metod) + evidensklass + namn + runsten (sanningskälla). rank_reason = läsbar.
...
LANGUAGE sql STABLE AS $$
  WITH deg AS (  -- grad per entitet i kunskapsgrafen (in+ut) = länk-auktoritet
  ...
  sig AS (
    SELECT *,
      least(1.0, base_sig
        + least(authority/20.0, 0.30)                                   -- graf-auktoritet (cap 0.30)
