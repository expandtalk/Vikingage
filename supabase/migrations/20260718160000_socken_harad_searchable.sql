-- Gör socken/härad SÖKBARA och VISNINGSBARA (DB-TODO item 4, utan polygoner).
-- (a) Vyn runic_with_coordinates exponerar r.socken + r.harad (append sist).
-- (b) search_inscriptions_flexible matchar även socken/harad → "Adelsö",
--     "Vallentuna härad" osv. hittar stenarna. Signaturen är oförändrad
--     (funktionen returnerar bara signum-listan som klienten filtrerar på).
-- Kör i SQL-editorn, sedan: supabase migration repair --status applied 20260718160000

-- (a) VY: lägg till socken + harad sist (CREATE OR REPLACE tillåter tillägg sist).
create or replace view public.runic_with_coordinates as
 SELECT r.id, r.signum, r.transliteration, r.location, r.parish, r.province,
    r.country, r.municipality, r.county, r.landscape, r.translation_en,
    r.translation_sv, r.object_type, r.period_start, r.period_end, r.dating_text,
    r.created_at,
    r.coordinates AS original_coordinates,
    c.latitude AS coordinates_latitude, c.longitude AS coordinates_longitude,
    ac.latitude AS additional_latitude, ac.longitude AS additional_longitude,
    ac.source AS coordinate_source, ac.confidence,
        CASE WHEN r.coordinates IS NOT NULL THEN 'original_coordinates'::text
             WHEN c.latitude IS NOT NULL THEN 'coordinates_table'::text
             WHEN ac.latitude IS NOT NULL THEN 'additional_coordinates'::text
             ELSE 'no_coordinates'::text END AS coordinate_status,
        CASE WHEN r.coordinates IS NOT NULL OR c.latitude IS NOT NULL OR ac.latitude IS NOT NULL THEN 'has_coordinates'::text
             ...
             ELSE 'low_geocoding_potential'::text END AS geocoding_priority,
    r.coord_confidence,
    r.coord_source,
    r.socken,
    r.harad
...
-- (b) RPC: lägg till socken/harad i den vanliga sökgrenen (oförändrad signatur).
...
BEGIN
  -- ✅ Kolla om det är en landskapssökning
  ...
  WHERE
    CASE
      ...
      ELSE (
        ...
        -- NYTT: socken + härad (auktoritativa rundata-kolumner)
